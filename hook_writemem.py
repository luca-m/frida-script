...
def prepare_script(frida_target_process):
    with open('hook_writemem.js','r') as f:
        return frida_target_process.session.create_script(f.read())

def on_frida_message(msg, data , chunks = CHUNKS):
    global COUNT
    if msg['payload'].has_key('writemem'):
        print("[{}] WriteProcessMemory {} ".format(msg['payload']['writemem']['time'],COUNT))
        with open('{}.mem'.format(COUNT)) as f:
            f.write(msg['payload']['writemem']['buff'])
        COUNT+=1
...
def main():
    ...
    parser.add_argument("-f","--runfile", type=str, help="target file to launch")
    ...
    pid=args.pid
    if args.runfile:
        prc=subprocess.Popen(args.runfile)
        pid=prc.pid
